class HolidaysController < ApplicationController
  ...
  def index
    ...
    @year = Time.new.year
    @dynamics = Holiday.dynamic_holidays(@year)
  ...
  def create
    ...
    respond_to do |format|
      if @holiday.save
        format.html { redirect_to(:action => 'index', :notice => 'Holiday was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @holiday.update_attributes(params[:holiday])
        format.html { redirect_to(:action => 'index', :notice => 'Holiday was successfully updated.') }
        format.xml  { head :ok }
      else
